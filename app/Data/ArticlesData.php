    ...
    public static function getArticles(): array
    {
        return [
            ...
            fonctionnalités...",
                ...
                'category' => 'Framework',
                'summary' => 'Découvrez pourquoi Laravel est le framework PHP le plus populaire',
        ...
        peuvent être définies pour différentes méthodes HTTP (GET, POST, PUT, DELETE) et peuvent contenir des paramètres...",
                'author' => 'Jean Martin',
                'published_at' => '2025-03-20',
                'category' => 'Laravel',
                'summary' => 'Apprenez à créer et organiser les routes de votre application Laravel',
...
2 / 8
atelier_laravel_route_controller.md
 2025-04-17
...
{
    // Fetch all articles with the matching category
    $articles = collect($this->getArticles())->filter(function ($article) use ($category) {
        return strtolower($article['category']) === strtolower($category);
    });

    return view('client.filteredArticles', ['articles' => $articles, 'category' => $category]);
}
    ...
    {
        return collect(self::getArticles())->pluck('category')->unique()->values();
    }
